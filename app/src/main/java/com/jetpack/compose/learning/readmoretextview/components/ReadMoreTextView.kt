...
//                default:  collapse option will always be visible
                if (enableCollapse) {
                    if (enableActionIcons) {
                        actionIcon = mapOf(Constants.ACTION_ICON_ID to InlineTextContent(Placeholder(24.sp, 24.sp, PlaceholderVerticalAlign.TextCenter)) {
    ...
    Box(modifier = Modifier.fillMaxWidth()) {
        MyClickableText(
            ...
            onClick = { offset ->
                actionText.getStringAnnotations(Constants.EXPAND, offset, offset).firstOrNull()?.let {
